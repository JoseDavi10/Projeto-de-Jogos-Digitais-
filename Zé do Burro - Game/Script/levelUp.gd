...
func _on_Player_level(value):
	...
	axeLvl = value[1];
	daggerLvl = value[2];
	$GridContainer2/Spear.text = "Aumentar poder da lança: " + str(spearLvl);
	$GridContainer2/Axe.text = "Machado lvl: " + str(axeLvl);
	$GridContainer2/mama.text = "Punhal lvl: " + str(daggerLvl);
	$lvlUPMusique.play()
